...
const SingleJourneyDefinition: React.FC = () => {
  ...
  const handleSendRequest = async () => {
    if (!guid.trim()) {
      setError('Please enter a Journey GUID');
      return;
    }

    if (!validateGuid(guid.trim())) {
      setError('Please enter a valid GUID format (e.g., 7a2ec01f-a178-42aa-85f2-47bfa00014a0)');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${vpbEndpoint}/journeys/${guid.trim()}`, {
        method: 'GET',
        headers: {
  ...
  const handleDownloadJson = () => {
    ...
    link.href = url;
    link.download = `single_journey_${guid}_${new Date().toISOString().slice(0, 10)}.json`;
      ...
      <div className="bg-white/60 dark:bg-secondary-800/60 backdrop-blur-sm rounded-xl p-6 
        ...
        <div className="flex gap-3 mb-4">
          <div className="flex-1">
            <label htmlFor="journey-guid" className="block text-sm font-medium text-secondary-700 dark:text-secondary-300 mb-2">
              Journey GUID <span className="text-red-500">*</span>
            </label>
            <input
              id="journey-guid"
              type="text"
              ...
              placeholder="e.g., 7a2ec01f-a178-42aa-85f2-47bfa00014a0"
          ...
          <div className="flex items-end">
            <button
              ...
              className="px-6 py-2 bg-primary-600 hover:bg-primary-700 disabled:bg-secondary-400 
...
export default SingleJourneyDefinition;